...
function applyNarratorStatePatch(state, patch) {
  ...
  // event_summary + witnesses go into the event log so future narrator turns
  // see what just happened. Without this, the log only sees scripted/mechanical
  // events and the narrator forgets its own beats turn-over-turn.
  const summary = patch.event_summary || patch.eventSummary;
  if (typeof summary === "string" && summary.trim()) {
    const witnesses = uniqueCompact(patch.witnesses || patch.witness_ids || []);
    next = appendEvent(next, summary.trim(), witnesses);
  }
...
// ============================================================================
// NARRATOR SYSTEM PROMPT (mirrors narrator_harness.jsx DEFAULT_PROMPT)
// ============================================================================
// This is the locked starting point. Iteration on the prompt itself happens in
// the harness; the artifact mirrors that prompt so the live loop and the test
// rig are pointed at the same contract.

const NARRATOR_SYSTEM_PROMPT = `You are the narrator for "Student Body," a single-player narrative dating sim. You describe scenes in third-person-limited from the protagonist's perspective. The protagonist is a male college freshman; the player controls his choices.
...
async function requestNarratorScene(state, action, systemPrompt = NARRATOR_SYSTEM_PROMPT) {
  const startTime = Date.now();
  const context = buildNarratorContext(state, action);
  const requestBody = {
  ...
  if (typeof window === "undefined" || !systemPrompt) {
    diagnostics.latencyMs = Date.now() - startTime;
    return { diagnostics, narration: "", choices: [], open: false, statePatch: null, error: "No window or system prompt" };
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      diagnostics.rawText = errorText;
      throw new Error(`Narrator request failed (${response.status}): ${errorText.slice(0, 240)}`);
    }
    ...
    const text = (data?.content || [])
      ...
      .join("\n")
      .trim();
    diagnostics.rawText = text;
    diagnostics.rawData = data;
    if (!text) {
      return { diagnostics, narration: "", choices: [], open: false, statePatch: null, error: "Empty response" };
    }
    const parsed = parseNarratorResponse(text);
    return { ...parsed, diagnostics };
  } catch (error) {
    diagnostics.latencyMs = diagnostics.latencyMs || (Date.now() - startTime);
    console.error("Narrator call failed:", error);
    return {
      diagnostics,
...
// ============================================================================
// FLIGHT RECORDER (TRACE LOG)
// ============================================================================
// Captures every player input, every state mutation, every narrator round-trip
// (full prompt + response + latency), every notification, and every handoff
// trigger. Each event includes trimmed before/after state snapshots so the
// session is fully reproducible.
//
// Stored separately from game state so it does not bloat saves. Capped at
// TRACE_MAX events with oldest evicted. Writes are debounced.
//
// Access: TraceLog.record({...}), TraceLog.getAll(), TraceLog.clear(),
// TraceLog.subscribe(fn), and download/copy helpers below.

const TRACE_KEY = "student-body:v1:trace";
const TRACE_MAX = 200;
const TRACE_WRITE_DEBOUNCE_MS = 1000;

// Strip the SVG asset registries from a state snapshot. They're static and
// huge; keeping them in every event would 3-5x the trace size for no benefit.
// If the player is testing a future asset swap, the registry version is
// implicit in the artifact code itself.
function trimStateForTrace(state) {
  if (!state || typeof state !== "object") return state;
  // Deep clone via JSON to detach the snapshot from the live object.
  // World-pack `raw` field can contain large data; drop it. Otherwise keep
  // everything that mutates with gameplay.
  try {
    const cloned = JSON.parse(JSON.stringify(state, (key, value) => {
      if (key === "raw" && value && typeof value === "object") return undefined;
      return value;
    }));
    return cloned;
  } catch {
...
const TraceLog = (() => {
  ...
  function scheduleWrite() {
    ...
    writeTimer = setTimeout(async () => {
      try {
        await window.storage.set(TRACE_KEY, JSON.stringify(events));
      } catch (error) {
        // If the trace exceeds storage limits, halve it and try once more.
        // This shouldn't normally fire because of TRACE_MAX, but defensively
        // handle the 5MB-per-key cap.
        try {
          events = events.slice(-Math.floor(TRACE_MAX / 2));
          await window.storage.set(TRACE_KEY, JSON.stringify(events));
          notify();
        } catch {}
      }
    }, TRACE_WRITE_DEBOUNCE_MS);
  }

  async function load() {
    if (loaded) return events;
    loaded = true;
    if (typeof window === "undefined" || !window.storage) return events;
    try {
      const r = await window.storage.get(TRACE_KEY);
  ...
  function record(event) {
    ...
    if (events.length > TRACE_MAX) {
      events = events.slice(-TRACE_MAX);
    }
    notify();
    scheduleWrite();
    return enriched;
  ...
  async function clear() {
    ...
    if (typeof window !== "undefined" && window.storage) {
      try { await window.storage.delete(TRACE_KEY); } catch {}
    }
  }

  return { load, record, getAll, getCount, subscribe, clear };
})();
...
function buildTraceJSON(events) {
  const out = {
    schema: "student-body-trace-v1",
...
function summarizeEvent(event) {
  ...
  switch (kind) {
    case "player_choice":
      return `Choice: "${event.choice?.label || event.choice?.id || "?"}"`;
    case "player_freeform":
      return `Freeform: "${event.text || ""}"`;
    case "narrator_request":
      return `Narrator request (${event.action?.source || "?"}): "${event.action?.label || ""}"`;
    case "narrator_response":
      return `Narrator response${event.error ? ` (ERROR: ${event.error})` : ""} — ${event.latencyMs ?? "?"}ms`;
    case "scripted_choice_applied":
      return `Scripted choice applied: ${event.choice?.label || "?"}`;
    case "navigate":
      return `Navigated to ${event.locationKey} via ${event.transitMode || "walk"}`;
    case "send_message":
      return `Sent Pulse to ${event.npcId}: ${event.templateId}`;
    case "commit_bulletin":
      return `Committed to bulletin item ${event.flyerId}`;
    case "add_note":
      return `Added Margin note: "${(event.text || "").slice(0, 60)}${(event.text || "").length > 60 ? "..." : ""}"`;
    case "set_alarm":
      return `Set alarm to slot ${event.alarmSlot}`;
    case "sleep":
      return `Slept until alarm`;
    case "load_pack":
      return `Loaded world pack: ${event.packId || event.packLabel || "?"}`;
    case "notification":
      return `Notification: ${event.notification?.app || "?"} — ${event.notification?.body || ""}`;
    case "narrator_handoff":
      return `Narrator handoff triggered`;
    case "new_game":
...
function buildTraceMarkdown(events) {
  const header = [
    "# Student Body — Session Trace",
    "",
  ...
  const sections = events.map((event, index) => {
    const lines = [];
    const gameTime = event.gameTime ? ` — _W${event.gameTime.week} ${event.gameTime.dayName} ${event.gameTime.slotLabel}_` : "";
    lines.push(`## ${index + 1}. \`${event.kind}\` — ${event.t}${gameTime}`);
    lines.push("");
    lines.push(`**${summarizeEvent(event)}**`);
    lines.push("");

    if (event.kind === "narrator_request") {
      lines.push("### System prompt");
      lines.push("```");
      lines.push((event.systemPrompt || "").trim());
      lines.push("```");
      lines.push("");
      lines.push("### Context payload");
      lines.push("```");
      lines.push((event.context || "").trim());
      lines.push("```");
      lines.push("");
    }

    if (event.kind === "narrator_response") {
      if (event.error) {
        lines.push(`### Error`);
        lines.push("```");
      ...
      if (event.rawText) {
        lines.push(`### Raw response (${event.latencyMs ?? "?"}ms)`);
        lines.push("```");
      ...
      if (event.parsed) {
        lines.push("### Parsed");
        lines.push("```json");
        lines.push(JSON.stringify(event.parsed, null, 2));
        lines.push("```");
        lines.push("");
      }
    }
    ...
    if (event.statePatch) {
      lines.push("### State patch");
      lines.push("```json");
      lines.push(JSON.stringify(event.statePatch, null, 2));
      lines.push("```");
      lines.push("");
    }
...
function SceneTextPanel({ scene, onChoice, onFreeform, dimmed, loading, narratorError }) {
  const [draft, setDraft] = React.useState("");
  const acceptsFreeform = scene?.open || (scene?.allowFreeform ?? false);
  const proseRef = React.useRef(null);
  const choicesRef = React.useRef(null);
  const freeformRef = React.useRef(null);
  const containerRef = React.useRef(null);
  const scrollAnimRef = React.useRef(null); // rAF id for in-flight smooth scrolls
  const [overflow, setOverflow] = React.useState({ top: false, bottom: false });
  // Panel height in px once measured. "auto" only on the very first render.
  const [panelHeight, setPanelHeight] = React.useState("auto");

  // Cap as fraction of the parent (image area, which now fills the window).
  // 55% leaves the world image visible above the panel; long beats scroll
  // inside the panel rather than pushing past the cap.
  const HEIGHT_CAP_FRAC = 0.55;
  const MIN_PX = 140;
  // Spacing between the prose section and the controls section.
  const SECTION_GAP = 14;

  // Padding sizes: the panel as a whole has vertical padding only. The prose
  // section gets horizontal padding too (so text doesn't kiss the edges) but
  // its container is invisible. The controls section has its own padding to
  // visually separate it.
  const PANEL_PAD_TOP = 18;
  const PANEL_PAD_BOTTOM = 18;
  const PANEL_PAD_X = 32;

  // Measure prose, choices, freeform individually and sum. Works regardless
  // of the current panel height because scrollHeight reflects natural content
  // height even when the prose is in overflow-y:auto mode.
  React.useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      const cap = Math.floor(parentH * HEIGHT_CAP_FRAC);

      const proseH = prose.scrollHeight;
      const choicesH = choicesRef.current ? choicesRef.current.offsetHeight : 0;
      const freeformH = freeformRef.current ? freeformRef.current.offsetHeight : 0;
      const innerGaps =
        (choicesH > 0 ? SECTION_GAP : 0) +
        (freeformH > 0 && (choicesH > 0 || proseH > 0) ? SECTION_GAP : 0);

      const desired = Math.max(
        MIN_PX,
        PANEL_PAD_TOP + PANEL_PAD_BOTTOM + proseH + choicesH + freeformH + innerGaps
      );
      const finalHeight = Math.min(desired, cap);
      setPanelHeight(`${finalHeight}px`);
    };
    ...
    if (typeof ResizeObserver !== "undefined") {
      ...
      ro.observe(prose);
      if (choicesRef.current) ro.observe(choicesRef.current);
      if (freeformRef.current) ro.observe(freeformRef.current);
      const parent = container.parentElement;
  ...
  }, [scene, loading, narratorError, draft, acceptsFreeform]);

  // Auto-scroll prose to top on each new scene's narration.
  const narrationKey = scene?.narration || "";
  React.useEffect(() => {
    if (proseRef.current) {
      // Cancel any in-flight smooth scroll, then jump to top.
      if (scrollAnimRef.current) cancelAnimationFrame(scrollAnimRef.current);
      proseRef.current.scrollTop = 0;
      checkOverflow();
    }
  ...
  React.useEffect(() => {
    checkOverflow();
  }, [checkOverflow, scene, panelHeight]);

  // Smooth scroll: animates scrollTop toward a target with ease-out. The
  // wheel handler accumulates target rather than snapping, so a fast spin
  // produces continuous motion instead of stuttering jumps.
  const animateScroll = React.useCallback((target) => {
    const el = proseRef.current;
    if (!el) return;
    ...
    const duration = 180; // ms; balance between snappy and smooth
    if (scrollAnimRef.current) cancelAnimationFrame(scrollAnimRef.current);
  ...
  // Wheel handler: each notch nudges the target by ~24px. Multiple notches
  // in quick succession accumulate before the animation finishes, which
  // produces the "continuous" feel of a fast scroll.
  const wheelTargetRef = React.useRef(null);
  const handleWheel = React.useCallback((e) => {
    ...
    // Use accumulated target if an animation is in flight; otherwise start
    // from current scrollTop. This way fast spins don't get stuck behind
    // unfinished animations.
    const base = (scrollAnimRef.current && wheelTargetRef.current != null)
      ? wheelTargetRef.current
      : el.scrollTop;
    const target = base + sign * stepPx;
    wheelTargetRef.current = target;
    animateScroll(target);
  }, [animateScroll]);

  // Cleanup any in-flight animation when the component unmounts.
  React.useEffect(() => {
    return () => {
      if (scrollAnimRef.current) cancelAnimationFrame(scrollAnimRef.current);
    };
  ...
  return (
    <div
      ...
      style={{
        ...
        height: panelHeight,
        // Translucent dark backdrop. The world hints through; prose sits
        // on top of it. No internal panel chrome here — the prose is full
        // width, the controls section below has its own visual styling.
        background: "rgba(20,18,16,0.75)",
        backdropFilter: "blur(8px)",
        WebkitBackdropFilter: "blur(8px)",
        borderTop: `1px solid ${PAL.dialogueBorder}`,
        boxShadow: "0 -16px 48px rgba(0,0,0,0.42)",
        // Vertical padding only — prose section bleeds horizontally to the
        // panel edges so the prose feels like text on the world rather than
        // text in a small box-within-a-box.
        padding: `${PANEL_PAD_TOP}px 0 ${PANEL_PAD_BOTTOM}px 0`,
        display: "flex", flexDirection: "column",
        gap: SECTION_GAP,
        color: PAL.ink,
        fontFamily: "Georgia, 'Times New Roman', serif",
        opacity: dimmed ? 0.55 : 1,
        transition: "height 0.22s ease-out, opacity 0.2s",
        zIndex: 15,
        boxSizing: "border-box",
        overflow: "hidden",
      }}
    >
      {/* Prose section — full width, no inner container styling. The text
          breathes from edge to edge with a comfortable horizontal padding
          for readability but no visible box around it. */}
      <div style={{
        position: "relative",
        flex: "1 1 auto",
        minHeight: 0,
        // Horizontal padding here only — gives the text breathing room from
        // the panel edges without making it look boxed.
        ...
        <div
          ...
          style={{
            maxHeight: "100%",
            overflowY: "auto",
            // Slight right padding to prevent text from kissing the scrollbar
            // when one appears.
            paddingRight: 4,
            fontSize: 15.5, lineHeight: 1.62,
            whiteSpace: "pre-wrap",
            // Hide the native scrollbar visually; the fade gradients handle
            // the affordance. Falls back to native scrollbar in browsers
            // without webkit support, which is fine.
            scrollbarWidth: "thin",
            scrollbarColor: "rgba(200,161,101,0.30) transparent",
          }}
        >
          {scene?.narration}
          {loading && (
            <span style={{
              ...
              color: PAL.accent,
              animation: "sb-narrator-pulse 1.2s ease-in-out infinite",
            }}>
              ...
            </span>
          )}
          {narratorError && (
            <div style={{
              ...
              Narrator call failed: {narratorError}
            </div>
          )}
        </div>
        {/* Top fade indicator — content above when scrolled */}
        <div style={{
          position: "absolute", left: PANEL_PAD_X, right: PANEL_PAD_X, top: 0, height: 24,
          pointerEvents: "none",
          background: "linear-gradient(to bottom, rgba(20,18,16,0.85), rgba(20,18,16,0))",
          opacity: overflow.top ? 1 : 0,
          transition: "opacity 0.18s",
        }} />
        {/* Bottom fade indicator — content below */}
        <div style={{
          position: "absolute", left: PANEL_PAD_X, right: PANEL_PAD_X, bottom: 0, height: 28,
          pointerEvents: "none",
          background: "linear-gradient(to top, rgba(20,18,16,0.85), rgba(20,18,16,0))",
          opacity: overflow.bottom ? 1 : 0,
          transition: "opacity 0.18s",
        }} />
      </div>

      {/* Controls section — choices and freeform. Visually distinct from
          the prose: has horizontal padding to align with the prose's text
          column, plus its own vertical breathing room. */}
      {((scene?.choices && scene.choices.length > 0) || (acceptsFreeform && onFreeform)) && (
        <div style={{
          flexShrink: 0,
          paddingLeft: PANEL_PAD_X,
          paddingRight: PANEL_PAD_X,
          ...
          {scene?.choices && scene.choices.length > 0 && (
            <div ref={choicesRef} style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {scene.choices.map(c => (
                <button
                  key={c.id}
                  disabled={loading}
          ...
          {/* Freeform input */}
          {acceptsFreeform && onFreeform && (
            <div ref={freeformRef} style={{ display: "flex", gap: 8 }}>
              <input
                value={draft}
                ...
                placeholder={loading ? "Narrator is writing..." : "Type what you do..."}
                disabled={loading}
                style={{
                  flex: 1,
              ...
              <button
                ...
                style={{
                  ...
                  background: draft.trim() && !loading ? "rgba(200,161,101,0.22)" : "rgba(200,161,101,0.06)",
...
function TraceBadge({ count, onClick }) {
  const nearCap = count >= TRACE_MAX * 0.85;
  const atCap = count >= TRACE_MAX;
  const tone = atCap ? "#fca5a5" : nearCap ? "#fcd34d" : "#c8a165";
  return (
    <button
      onClick={onClick}
      title={`Flight recorder — ${count} event${count === 1 ? "" : "s"} captured. Click to open.`}
      style={{
    ...
    >
      <span style={{
        ...
        background: tone,
        boxShadow: atCap ? "none" : `0 0 6px ${tone}`,
      }} />
...
function TracePanel({ onClose }) {
  const [events, setEvents] = useState(() => TraceLog.getAll());
  const [filter, setFilter] = useState("all");
  const [confirmingClear, setConfirmingClear] = useState(false);
  const [copyState, setCopyState] = useState("idle"); // idle | copied | failed

  useEffect(() => {
    const unsubscribe = TraceLog.subscribe(next => setEvents(next.slice()));
    return unsubscribe;
  }, []);
  ...
  const filtered = filter === "all"
    ? events
    : events.filter(e => {
        if (filter === "narrator") return e.kind === "narrator_request" || e.kind === "narrator_response";
        if (filter === "input") return e.kind === "player_choice" || e.kind === "player_freeform";
        if (filter === "errors") return e.error || (e.kind === "narrator_response" && e.error);
        return true;
      });

  const handleDownloadJson = () => {
    downloadBlob(`student-body-trace-${Date.now()}.json`, "application/json", buildTraceJSON(events));
  };
  const handleDownloadMd = () => {
    downloadBlob(`student-body-trace-${Date.now()}.md`, "text/markdown", buildTraceMarkdown(events));
  };
  const handleCopy = async () => {
    const ok = await copyToClipboard(buildTraceJSON(events));
  ...
  const handleClear = async () => {
    ...
    await TraceLog.clear();
    setConfirmingClear(false);
  };

  const sessionStart = events.find(e => e.kind === "session_start")?.t;
  const newestEvent = events[events.length - 1]?.t;
  ...
  const buttonStyle = {
    ...
    fontWeight: 600,
    background: "rgba(200,161,101,0.12)",
    border: "1px solid rgba(200,161,101,0.34)",
    borderRadius: 6,
    color: PAL.ink,
    cursor: "pointer",
  };
    ...
    >
      <div
        ...
        style={{
          ...
          border: `1px solid ${PAL.dialogueBorder}`,
          borderRadius: 12,
          display: "flex", flexDirection: "column",
          overflow: "hidden",
          color: PAL.ink,
      ...
      >
        ...
        <div style={{
          padding: "14px 18px",
          borderBottom: `1px solid ${PAL.dialogueBorder}`,
          display: "flex", alignItems: "center", gap: 12,
        }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 14, fontWeight: 700, letterSpacing: 0.3 }}>
              Flight Recorder · {events.length} / {TRACE_MAX}
            </div>
            <div style={{ fontSize: 10, color: PAL.inkDim, marginTop: 2 }}>
              {sessionStart ? `Session start: ${sessionStart}` : "No session start recorded"}
              {newestEvent && newestEvent !== sessionStart ? ` · Newest: ${newestEvent}` : ""}
            </div>
          </div>
          <button onClick={onClose} style={{
            ...buttonStyle, padding: "4px 10px", fontSize: 16, lineHeight: 1,
          }}>×</button>
        </div>
        ...
        <div style={{
          ...
          {[
            ...
            <button
              ...
              style={{
                ...buttonStyle,
                background: filter === f.id ? "rgba(200,161,101,0.30)" : "rgba(200,161,101,0.06)",
                borderColor: filter === f.id ? PAL.iconBorderActive : PAL.iconBorder,
              }}
            >
              {f.label}
            </button>
          ...
          <div style={{ flex: 1 }} />
          <button onClick={handleDownloadJson} style={buttonStyle}>Download JSON</button>
          <button onClick={handleDownloadMd} style={buttonStyle}>Download Markdown</button>
          <button onClick={handleCopy} style={buttonStyle}>
            {copyState === "copied" ? "✓ Copied" : copyState === "failed" ? "✗ Failed" : "Copy JSON"}
          </button>
          <button
            onClick={handleClear}
            style={{
              ...buttonStyle,
              background: confirmingClear ? "rgba(248,113,113,0.20)" : "rgba(248,113,113,0.08)",
              borderColor: confirmingClear ? "#fca5a5" : "rgba(248,113,113,0.34)",
              color: confirmingClear ? "#fecaca" : "#fca5a5",
            }}
          >
        ...
        <div style={{ flex: 1, overflow: "auto", padding: "8px 18px 18px" }}>
          {filtered.length === 0 ? (
            <div style={{
              padding: "40px 20px", textAlign: "center",
              color: PAL.inkSoft, fontStyle: "italic",
            }}>
              No events match this filter.
            </div>
          ) : (
            filtered.slice().reverse().map((event, index) => (
              <TraceEventRow key={`${event.t}-${index}`} event={event} />
            ))
          )}
        </div>
      </div>
...
function TraceEventRow({ event }) {
  ...
  const isNarrator = event.kind === "narrator_request" || event.kind === "narrator_response";
  const tone = hasError ? "#fca5a5" : isNarrator ? "#c8a165" : PAL.inkDim;
  const time = event.t ? new Date(event.t).toLocaleTimeString(undefined, { hour12: false }) : "";
  const gameTimeStr = event.gameTime ? `W${event.gameTime.week} ${event.gameTime.dayName} ${event.gameTime.slotLabel}` : "";
  ...
  return (
    <div style={{
      ...
      >
        <span style={{ color: PAL.inkSoft, fontFamily: "ui-monospace, monospace", fontSize: 10, minWidth: 70 }}>
          {time}
        </span>
        <span style={{
          padding: "2px 6px",
        ...
        {gameTimeStr && (
          <span style={{ color: PAL.inkSoft, fontSize: 10 }}>{gameTimeStr}</span>
        )}
        {event.latencyMs != null && (
          <span style={{ color: PAL.inkSoft, fontSize: 10, fontFamily: "ui-monospace, monospace" }}>
            {event.latencyMs}ms
          </span>
        )}
        <span style={{ color: PAL.inkSoft, fontSize: 11 }}>{expanded ? "▾" : "▸"}</span>
      </button>
      {expanded && (
        <div style={{
          marginTop: 6, marginLeft: 80,
          padding: 10,
...
export default function StudentBody() {
  ...
  // Narrator state. narratorScene caches the most recent live narrator
  // response so the dialogue strip can render it. Loading is true while a
  // request is in flight; error holds the last error message (cleared on
  // next successful turn).
  const [narratorScene, setNarratorScene] = useState(null);
  const [narratorLoading, setNarratorLoading] = useState(false);
  const [narratorError, setNarratorError] = useState(null);

  // Flight recorder state. traceCount drives the badge; tracePanelOpen is
  // the panel toggle. The trace itself lives in the TraceLog singleton.
  ...
  useEffect(() => {
    (async () => {
      ...
      setLoaded(true);
      // Hydrate the trace from storage and start tracking count.
      await TraceLog.load();
      setTraceCount(TraceLog.getCount());
      TraceLog.record({
        kind: "session_start",
        userAgent: typeof navigator !== "undefined" ? navigator.userAgent : null,
      });
    })();
    // Subscribe to trace updates for badge count.
    const unsubscribe = TraceLog.subscribe(() => setTraceCount(TraceLog.getCount()));
    return unsubscribe;
  }, []);
  ...
  const handleNewGame = useCallback(async () => {
    ...
    TraceLog.record({
      kind: "new_game",
      gameTime: state ? getCalendarMoment(state) : null,
      stateBefore: state ? trimStateForTrace(state) : null,
    });
    await clearState();
    setState(makeFreshState());
    setPhone({ open: false, view: "home", orientation: "portrait" });
    setNotif(null);
    setNarratorScene(null);
  ...
  const handleNavigate = useCallback((locationKey, transitMode = "walk") => {
    if (!state) return;
    const before = trimStateForTrace(state);
    ...
    TraceLog.record({
      ...
      locationKey,
      transitMode,
      gameTime: getCalendarMoment(state),
      notification: update.notification || processed.notification || null,
      stateBefore: before,
      stateAfter: trimStateForTrace(processed.state),
    });
  ...
  // The narrator takes over after the scripted intro reaches "met Mari."
  // Before that, scripted scenes carry the bootstrap so the player has
  // something to do without a live model call. After that, every turn
  // (choice or free-form) goes through the narrator.
  const shouldUseNarrator = useCallback((s) => {
    return Boolean(s?.metMari);
  }, []);

  const runNarratorTurn = useCallback(async (currentState, action) => {
    setNarratorLoading(true);
    setNarratorError(null);
    const stateBefore = trimStateForTrace(currentState);
    const gameTime = getCalendarMoment(currentState);

    // Record the request as it goes out.
    const requestRecord = TraceLog.record({
      kind: "narrator_request",
      gameTime,
      action,
      systemPrompt: NARRATOR_SYSTEM_PROMPT,
      context: buildNarratorContext(currentState, action),
      stateBefore,
    });

    try {
      const result = await requestNarratorScene(currentState, action);
      const diagnostics = result?.diagnostics || {};
      const responseEvent = {
        kind: "narrator_response",
        gameTime,
        action,
        ...
        parsed: result ? {
          ...
          stateParseError: result.stateParseError ? String(result.stateParseError) : null,
        } : null,
        error: result?.error || null,
        requestEventTime: requestRecord.t,
      };

      if (!result || result.error) {
        TraceLog.record(responseEvent);
        setNarratorError(result?.error || "No response from narrator.");
        return null;
      }

      // Apply the structured patch to game state.
      let patched = currentState;
      if (result.statePatch) {
        patched = applyNarratorStatePatch(patched, result.statePatch);
      }
      patched = advanceTime(patched, DEFAULT_ACTION_CHUNKS);
      const processed = processTimedSystems(patched);
      setState(processed.state);

      const taggedChoices = (result.choices || []).map((c, i) => ({
        ...c,
        id: c.id || `narrator_${Date.now()}_${i}`,
        _narrator: true,
      }));
      setNarratorScene({
        narration: result.narration || "",
        choices: taggedChoices,
        open: result.open || taggedChoices.length === 0,
        allowFreeform: true,
      });

      // Record the response with after-state for full reproducibility.
      TraceLog.record({
        ...responseEvent,
        statePatch: result.statePatch,
        stateAfter: trimStateForTrace(processed.state),
      ...
      if (processed.notification) {
        setTimeout(() => showNotif(processed.notification), 400);
      }
      return result;
    } catch (error) {
      TraceLog.record({
      ...
      setNarratorError(error?.message || String(error));
      return null;
    } finally {
  ...
  const handleChoice = useCallback((choice) => {
    // Narrator-emitted choices route through the live narrator turn.
    if (choice._narrator) {
      if (!state || narratorLoading) return;
      TraceLog.record({
        kind: "player_choice",
        choice,
        gameTime: getCalendarMoment(state),
        narratorPath: true,
        stateBefore: trimStateForTrace(state),
      });
      runNarratorTurn(state, { label: choice.label, source: "choice" });
      return;
    }

    // Scripted choices use the existing deterministic path.
    setState(s => {
      const before = trimStateForTrace(s);
      const update = applyChoice(s, choice);
      const processed = processTimedSystems(update.state);
      const after = trimStateForTrace(processed.state);

      TraceLog.record({
        kind: "player_choice",
        choice,
        gameTime: getCalendarMoment(s),
        narratorPath: false,
        stateBefore: before,
      });
      TraceLog.record({
        kind: "scripted_choice_applied",
        choice,
        gameTime: getCalendarMoment(processed.state),
        notification: update.notification || processed.notification || null,
        stateBefore: before,
        stateAfter: after,
      });
      ...
      // Handoff: if this choice flips us into narrator territory (e.g. the
      // "met Mari" tag), kick off the first narrator turn from the post-
      // choice state so the player sees the live world immediately.
      if (shouldUseNarrator(processed.state) && !shouldUseNarrator(s)) {
        TraceLog.record({
          kind: "narrator_handoff",
          gameTime: getCalendarMoment(processed.state),
          triggerChoice: choice,
        });
        setTimeout(() => runNarratorTurn(processed.state, { label: choice.label, source: "handoff" }), 50);
      }

      return processed.state;
    });
  }, [showNotif, state, narratorLoading, runNarratorTurn, shouldUseNarrator]);

  const handleFreeform = useCallback((text) => {
    if (!state || narratorLoading) return;
    TraceLog.record({
      kind: "player_freeform",
      text,
      gameTime: getCalendarMoment(state),
      stateBefore: trimStateForTrace(state),
    });
    runNarratorTurn(state, { label: text, source: "freeform" });
  }, [state, narratorLoading, runNarratorTurn]);
  ...
  const handleSendMessage = useCallback((npcId, templateId) => {
    setState(s => {
      ...
      TraceLog.record({
        ...
        templateId,
        gameTime: getCalendarMoment(s),
        notification: update.notification || null,
        stateBefore: before,
        stateAfter: trimStateForTrace(next),
      });
      if (update.notification) setTimeout(() => showNotif(update.notification), 200);
      return next;
    });
  ...
  const handleAddNote = useCallback((text) => {
    setState(s => {
      const before = trimStateForTrace(s);
      const next = normalizeState(addMarginNote(s, text).state);
      TraceLog.record({
        kind: "add_note",
        text,
        gameTime: getCalendarMoment(s),
  ...
  const handleSetAlarm = useCallback((alarmSlot) => {
    setState(s => {
      const before = trimStateForTrace(s);
      const next = normalizeState(setWakeAlarm(s, alarmSlot));
      TraceLog.record({
        kind: "set_alarm",
        alarmSlot,
        gameTime: getCalendarMoment(s),
  ...
  // Scene resolver: scripted while pre-handoff, narrator scene afterward.
  // While the narrator is loading and we don't have a cached scene yet,
  // fall back to scripted so the screen isn't blank.
  const useNarrator = shouldUseNarrator(state);
  const scene = (useNarrator && narratorScene)
    ? narratorScene
    : getScriptedScene(state);
  ...
  return (
    <>
      ...
      <div style={{
        ...
        {/* The game window — single canvas, image area fills, scene panel
            overlays inside it */}
        <div style={{
          position: "relative", width: "100%", height: "100%",
          maxWidth: 1400, margin: "0 auto",
        }}>
          {/* Image area (full window) */}
          <div style={{
            position: "absolute", inset: 0,
            minHeight: 0,
            overflow: "hidden",
            ...
            {/* Scene text panel — overlays the bottom of the image area.
                Holds prose, choices, and freeform input. Dims with the world
                when the phone is open, since it's now part of the same layer. */}
            <SceneTextPanel
              scene={scene}
              onChoice={handleChoice}
              onFreeform={useNarrator ? handleFreeform : undefined}
          ...
          {/* Flight recorder badge — always visible, click to open panel */}
          <TraceBadge count={traceCount} onClick={() => setTracePanelOpen(true)} />
        </div>
      </div>

      {tracePanelOpen && <TracePanel onClose={() => setTracePanelOpen(false)} />}
    </>
  );
}
